    ...
    <div class="config-card config-card--clickable" @click="modelsOpen = true">
      <div class="config-card__row">
        ...
        <div class="config-card__row-text">
          ...
          <div class="config-card__hint">
            <template v-if="wikiGlobalModelId && activeStepCount > 0">Wiki default set; {{ activeStepCount }} steps override it</template>
            <template v-else-if="wikiGlobalModelId">Wiki default set and inherited by all steps</template>
            <template v-else-if="activeStepCount > 0">{{ activeStepCount }} steps use custom models</template>
            <template v-else>All steps use the system default model</template>
          </div>
        </div>
    ...
    <div class="config-card config-card--clickable" @click="rulesOpen = true">
      <div class="config-card__row" style="align-items: flex-start">
        ...
        <div class="config-card__row-text" style="flex:1;min-width:0">
          <div class="config-card__title">Processing rules</div>
          <div class="config-card__hint">Quality, format, and language rules used while AI processes source material</div>
          <!-- Preview snippet -->
          <div v-if="configContent.trim()" class="rules-snippet">
            ...
            <span v-if="totalLines > 4" class="rules-snippet__more">+{{ totalLines - 4 }} lines</span>
          </div>
          <div v-else class="rules-snippet rules-snippet--empty">Click to configure →</div>
        </div>
